...
// proposalPath returns the directory path for a proposal UUID without
// containment checking. Callers that take an id from external input must use
// resolveProposalDir; proposalPath is reserved for trusted ids generated by
// NewProposalMetadata (uuid.New().String()).
