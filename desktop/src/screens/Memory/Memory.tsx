...
import { Plus, Trash2 as Trash, RefreshCw as Refresh } from "lucide-react";
...
function Memory({ profile }: { profile?: string }): React.JSX.Element {
  ...
  const loadData = useCallback(async () => {
    try {
      const memoryData = await fetchJSON<MemoryData>(`/v1/memory${profileParam}`);
      setData(memoryData);
  ...
  async function handleSaveUserProfile(): Promise<void> {
    ...
    try {
      const result = await fetchJSON<OperationResult>(
        `/v1/memory/profile${profileParam}`,
        {
          method: "PUT",
