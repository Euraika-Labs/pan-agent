...
function Tools({ profile }: ToolsProps): React.JSX.Element {
  ...
  const loadToolsets = useCallback(async (): Promise<void> => {
    setLoading(true);
    try {
      const list = await fetchJSON<ToolsetInfo[]>(`/v1/tools${profileParam}`);
      setToolsets(list);
    } catch (err) {
  ...
  async function handleToggle(
    ...
    try {
      await fetchJSON(`/v1/tools/${encodeURIComponent(key)}${profileParam}`, {
        method: "PUT",
        body: JSON.stringify({ enabled: !currentEnabled }),
      });
    } catch (err) {
      console.error("[Tools] toggle error:", err);
