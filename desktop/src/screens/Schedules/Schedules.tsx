...
function Schedules({ profile }: SchedulesProps): React.JSX.Element {
  ...
  const loadJobs = useCallback(async (): Promise<void> => {
    try {
      const list = await fetchJSON<CronJob[]>(`/v1/cron${profileParam}`);
      setJobs(list);
    } catch {
      ...
      const result = await fetchJSON<OperationResult>(
        `/v1/cron${profileParam}`,
        {
          ...
          body: JSON.stringify({
            ...
            deliver: newDeliver !== "local" ? newDeliver : undefined,
          }),
        },
      );
      if (result.success) {
  ...
  async function handleRemove(jobId: string): Promise<void> {
    ...
    try {
      const result = await fetchJSON<OperationResult>(
        `/v1/cron/${encodeURIComponent(jobId)}${profileParam}`,
        { method: "DELETE" },
      );
  ...
  async function handleToggle(job: CronJob): Promise<void> {
    ...
    try {
      ...
      const result = await fetchJSON<OperationResult>(
        `/v1/cron/${encodeURIComponent(job.id)}/${action}${profileParam}`,
        { method: "POST" },
      );
      if (result.success) {
        await loadJobs();
  ...
  async function handleTrigger(jobId: string): Promise<void> {
    ...
    try {
      const result = await fetchJSON<OperationResult>(
        `/v1/cron/${encodeURIComponent(jobId)}/trigger${profileParam}`,
        { method: "POST" },
      );
      if (result.success) {
        await loadJobs();
